    ...
    <div className="max-w-5xl flex flex-col gap-6">
      ...
      <div className="flex items-center gap-2">
        {filters.map(([key, label]) => (
          <button key={key} onClick={() => setFilter(key)} className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors cursor-pointer ${filter === key ? "bg-gold/10 text-gold border border-gold/30" : "bg-white border border-border text-muted hover:text-text"}`}>
            {label}
          </button>
        ))}
      </div>
              ...
              <div key={item.id} className="bg-white rounded-xl border border-border overflow-hidden">
                ...
                <div className="px-5 py-4 flex items-start justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    {filter === "pending" && (
                      <>
                        <button onClick={() => { setModal({ id: item.id, action: "approve" }); setNote(""); }} disabled={busy} className="px-3 py-1.5 rounded-lg bg-emerald-600 text-white text-xs font-semibold hover:bg-emerald-700 transition-colors cursor-pointer disabled:opacity-50">{rq.approve}</button>
                        <button onClick={() => { setModal({ id: item.id, action: "reject" }); setNote(""); }} disabled={busy} className="px-3 py-1.5 rounded-lg border border-border text-xs font-semibold text-muted hover:text-red-600 hover:border-red-200 transition-colors cursor-pointer disabled:opacity-50">{rq.reject}</button>
                      </>
                    )}
                  </div>
                </div>
        ...
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl border border-border shadow-xl p-8 w-full max-w-md">
            ...
            <div className="mb-4">
              ...
              <textarea value={note} onChange={(e) => setNote(e.target.value)} rows={3} className="w-full rounded-lg border border-border px-3 py-2 text-sm focus:outline-none focus:border-gold"
                placeholder={modal.action === "approve" ? rq.notePlaceholderApprove : rq.notePlaceholderReject} />
            </div>
            <div className="flex items-center gap-3">
              <button onClick={handleAction} disabled={busy} className={`px-5 py-2 rounded-lg text-white text-sm font-semibold cursor-pointer disabled:opacity-50 ${modal.action === "approve" ? "bg-emerald-600 hover:bg-emerald-700" : "bg-red-600 hover:bg-red-700"}`}>
                {busy ? rq.processing : modal.action === "approve" ? rq.confirmApproval : rq.confirmRejection}
              </button>
              <button onClick={() => setModal(null)} className="px-4 py-2 rounded-lg border border-border text-sm font-medium text-muted hover:text-text cursor-pointer">{t.common.cancel}</button>
            </div>
          </div>
        </div>
      )}
