  ...
  return (
    <Link href={href} className="group block">
      <div className="bg-white rounded-2xl border border-border shadow-sm px-7 py-6 flex flex-col gap-2 hover:border-gold/50 hover:shadow-md transition-all">
        ...
        <p className="text-4xl font-bold text-text leading-none group-hover:text-gold transition-colors">{value}</p>
  ...
  return (
    <Link href={href} className="group block">
      <div className="bg-white rounded-2xl border border-border p-8 flex flex-col gap-5 shadow-sm hover:shadow-md hover:border-gold/50 transition-all h-full">
        <div className="w-14 h-14 rounded-xl bg-[#FBF7F0] flex items-center justify-center text-gold">{icon}</div>
    ...
    <div className="max-w-5xl flex flex-col gap-10">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          ...
          <SectionCard title={d.placementsLabel} description={d.placementsDesc} href="/placements" cta={d.viewPlacements}
            icon={<svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="7" width="20" height="14" rx="2" ry="2" /><path d="M16 3l-4 4-4-4" /></svg>} />
